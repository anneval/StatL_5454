...
<!-- EXERCISE 1 -->
# Exercise 1
We load the data set \textit{Carseats} from package \textit{ISLR2}. We then select 100 samples as training data and use the remaining ones as test data.
```{r}
...
plot(tree_party, gp = gpar(fontsize = 6))
...
MSE(tree_fit)
```
Let's use cross-validation in order to determine the optimal level of tree complexity.
```{r}
...
plot(ptree_party, gp = gpar(fontsize = 10))
