...
# Function to preprocess and pad sequences
preprocess_data <- function(max_features, maxlen = 100) {
...
# Dictionary sizes to evaluate
dictionary_sizes <- c(500, 1000, 3000, 5000, 10000)
...
for (max_features in dictionary_sizes) {
  cat(sprintf("Training with dictionary size: %d\n", max_features))
  
  # Preprocess the data
  data <- preprocess_data(max_features)
  ...
  # Create the model
  model <- create_model(input_shape = dim(x_train)[2])
  ...
  cat(sprintf("Test loss: %.4f, Test accuracy: %.4f\n\n", score$loss, score$acc))
  
  # Store the results
  results[[as.character(max_features)]] <- score$acc
}
...
results_df <- data.frame(
  dictionary_size = as.numeric(names(results)),
...
ggplot(results_df, aes(x = dictionary_size, y = accuracy)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = dictionary_sizes) +
  labs(
    title = "Effect of Dictionary Size on Model Accuracy",
    x = "Dictionary Size",
    y = "Test Accuracy"
  ) +
  theme_minimal()
